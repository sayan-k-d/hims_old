...
class Welcome extends Front_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->library(['mailer', 'form_builder']);
        ...
        $this->data['banner_notices'] = $this->cms_program_model->getByCategory($ban_notice_type, ['start' => 0, 'limit' => 5]);
    ...
    {
        $this->load->view('homeScreen'); // same page for /homescreen
    }
    ...
    public function page($slug)
    {
        ...
        if (! empty($this->data['page']['category_content'])) {
            ...
            //get the posts data
            $this->data['page']['category_content'][$first_key] = $this->cms_program_model->getByCategory($content_array[$first_key], ['limit' => $this->perPage]);
    ...
    public function ajaxPaginationData()
    {
        ...
        $this->ajax_pagination->initialize($config);
        //get the posts data
        $data['category_content'] = $this->cms_program_model->getByCategory($page_content_type, ['start' => $offset, 'limit' => $this->perPage]);
        //load the view
    ...
    public function userlogin($username = null, $password = null)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $json_array = ['status' => '0', 'error' => $jsons];
            ...
            if (isset($login_details) && ! empty($login_details)) {
                ...
                if ($user->is_active == "yes") {
                    ...
                    if ($result[0]->lang_id != 0) {
                        $lang_array = ['lang_id' => $result['0']->lang_id, 'language' => $result['0']->language];
                    } else {
                        $lang_array = ['lang_id' => $setting_result[0]['lang_id'], 'language' => $setting_result[0]['language']];
                    }
                    ...
                    if ($result != false) {
                        if ($result[0]->role == "patient") {
                            ...
                            $session_data = [
                                ...
                                'language'               => ['lang_id' => $setting_result[0]['lang_id'], 'language' => $setting_result[0]['language']],
    ...
    public function appointment()
    {
        ...
        $this->data['page']          = ['title' => $this->lang->line("appointment"), 'meta_title' => '', 'meta_keyword' => '', 'meta_description' => ''];
    ...
    public function register()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $json_array = ['status' => '0', 'error' => $json];
            echo json_encode($json_array);
            die;
        } else {
            ...
            $array          = ['status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message'), 'id' => $insert_id];
            $sender_details = ['id' => $insert_id, 'credential_for' => 'patient', 'username' => $username, 'password' => $user_password, 'contact_no' => $this->input->post('phone'), 'email' => $this->input->post('email')];
    ...
    public function show_404()
    {
        ...
        $this->data['page']          = ['title' => '', 'meta_title' => '', 'meta_keyword' => '', 'meta_description' => ''];
        $patientpanel                = $this->customlib->patientpanel();
        $this->data['patientpanel']  = $patientpanel;
    ...
    public function chackvalidation()
    {
        ...
        if ($this->form_validation->run() == false) {
            $msg = ["message" => form_error('message')];
            if (! empty($custom_fields)) {
                foreach ($custom_fields as $custom_fields_key => $custom_fields_value) {
                    if ($custom_fields_value['validation']) {
                        $custom_fields_id = $custom_fields_value['id'];
            ...
            $data = ['status' => 'fail', 'error' => $error_msg, 'message' => ''];

        } else {
            $data = ['status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message')];
        }
    ...
    public function annual_calendar()
    {
        ...
        $this->data['page']          = ['title' => $this->lang->line("annual_calendar"), 'meta_title' => '', 'meta_keyword' => '', 'meta_description' => ''];
