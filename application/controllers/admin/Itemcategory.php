...
class Itemcategory extends Admin_Controller
{
    ...
    if (isset($total_rows) && !empty($total_rows)) {          
        ...
        foreach ($total_rows as $row_key => $row_value) {  
            ...
            $this->form_validation->set_rules('item_head_' . $row_value, 'Item Head', 'trim|required');

            if ($this->form_validation->run() == false) {           
                $msg = array(
                    'item_category' => form_error('itemcategory_' . $row_value),                        
                    'item_head'     => form_error('item_head_' . $row_value),
                );
        ...
        foreach ($total_rows as $row_key => $row_values) {                 
            $data  = array(
                ...
                'item_head'     => $this->input->post('item_head_' . $row_values),
    ...
    public function edit()
{
    ...
    $this->form_validation->set_rules('itemcategory', $this->lang->line('item_category'), 'trim|required|xss_clean');
    $this->form_validation->set_rules('item_head', 'Item Head', 'trim|required|xss_clean'); // ✅ added validation

    if ($this->form_validation->run() == false) {
        $msg = array(
            'itemcategory' => form_error('itemcategory'),
            'item_head'    => form_error('item_head'), // ✅ include this in response
        );
        ...
        $data = array(
            ...
            'item_category' => $this->input->post('itemcategory'),
            'description'   => $this->input->post('description'),
            'item_head'     => $this->input->post('item_head'), // ✅ include item_head
        );
