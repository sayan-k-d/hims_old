                    ...
                    <div class="box-header ptbnull">
                        ...
                        <div class="box-tools pull-right">
                            <?php if ($this->rbac->hasPrivilege('item_stock', 'can_add')) { ?>
                                <a data-toggle="modal" data-target="#myModal" class="btn common_btn btn-sm additemstock"><i class="fa fa-plus"></i> <?php echo $this->lang->line('add_item_stock'); ?></a>
                            <?php } ?>
                            <?php if ($this->rbac->hasPrivilege('issue_item', 'can_view')) { ?>
                                <a href="<?php echo base_url(); ?>admin/issueitem" class="btn common_btn btn-sm"><i class="fa fa-reorder"></i> <?php echo $this->lang->line('issue_item'); ?> </a>  
                                <?php
                            }
                            if ($this->rbac->hasPrivilege('item', 'can_view')) {
                                ?>
                                <a href="<?php echo base_url(); ?>admin/item" class="btn common_btn btn-sm"><i class="fa fa-reorder"></i> <?php echo $this->lang->line('item'); ?> </a> 
                            <?php } ?>
                        </div>
                    </div><!-- /.box-header -->					
                    <div class="box-body">
                            ...
                            <table class="table table-hover table-striped table-bordered ajaxlist" data-export-title="<?php echo $this->lang->line('item_stock_list'); ?>">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Salvage Value  </th>
                                        <th>Useful Life </th>
                                        <th>Depreciation  <?php echo " (".$currency_symbol.")"; ?></th>
                                    </tr>
                                </thead>
                                <tbody>
                                </tbody>
            ...
            <form id="form1" action="<?php echo base_url() ?>admin/itemstock/add" id="itemstockform" name="itemstockform" method="post" accept-charset="utf-8" enctype="multipart/form-data">
                <div class="scroll-area">
                    <div class="modal-body pt0 pb0">
                                    ...
                                    <select autofocus="" id="item_category_id" name="item_category_id" class="form-control" >
                                        ...
                                        foreach ($itemcatlist as $item_category) {
                                            ...
                                            <option value="<?php echo $item_category['id'] ?>" 
                                                    data-itemhead="<?php echo $item_category['item_head']; ?>"
                                                    <?php if (set_value('item_category_id') == $item_category['id']) echo "selected = selected"; ?>>
                        ...
                        <div class="col-md-6" id="expiry_date_group" style="display:none;">
                            <div class="form-group">
                        ...
                        <!-- Fields for Capital Equipment -->
                        <div class="col-md-6" id="salvage_value_group" style="display:none;">
                            <div class="form-group">
                                <label for="salvage_value">Salvage Value</label>
                                <input type="text" id="salvage_value" name="salvage_value" class="form-control">
                            </div>
                        </div>
                        
                        <div class="col-md-6" id="useful_life_group" style="display:none;">
                            <div class="form-group">
                        ...
                        <div class="col-md-6" id="annual_depreciation_group" style="display:none;">
                            <div class="form-group">
                                <label for="annual_depreciation">Annual Depreciation</label>
                                <input type="text" id="annual_depreciation" name="annual_depreciation" class="form-control" readonly>
                            </div>
                        </div>
                            <div class="col-md-12">
                                <div class="form-group">
                ...
                <div class="modal-footer">
                    <div class="pull-right">
                        <button type="submit" data-loading-text="<?php echo $this->lang->line('processing'); ?>" id="form1btn" class="btn common_btn pull-right"><i class="fa fa-check-circle"></i> <?php echo $this->lang->line('save'); ?></button>
                    </div>
                </div>
            </form>
        </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select autofocus="" id="edititem_category_id" name="item_category_id" class="form-control" >
                                    ...
                                    foreach ($itemcatlist as $item_category) {
                                        ?>
                                        <select id="edititem_category_id" name="item_category_id" class="form-control">
                                            <option value=""><?php echo $this->lang->line('select'); ?></option>
...
<script type="text/javascript">
    ...
    function calculateDepreciation() {
        ...
        if (usefulLife > 0 && purchasePrice >= salvageValue) {
            var annualDepreciation = (purchasePrice - salvageValue) / usefulLife;
            $('#annual_depreciation').val(annualDepreciation.toFixed(2));
        } else {
            $('#annual_depreciation').val('');
        }
    }
    ...
    $(document).ready(function () {
        ...
        var itemHead = selectedOption.data('itemhead');

        // Hide all conditional fields initially
        $('#expiry_date_group').hide();
        $('#salvage_value_group').hide();
        $('#useful_life_group').hide();
        $('#annual_depreciation_group').hide();

        if (itemHead === 'Consumables') {
            $('#expiry_date_group').show();
        } else if (itemHead === 'Capital Equipment') {
            $('#salvage_value_group').show();
            $('#useful_life_group').show();
            $('#annual_depreciation_group').show();
        }
    });
    
    $('#purchase_price, #salvage_value, #useful_life').on('input', function() {
    ...
    // Also run on page load in case values are prefilled
    calculateDepreciation();

    // Trigger once in case of form repopulation
