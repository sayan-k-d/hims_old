...
<div class="modal fade" id="addmultiplerow" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    ...
    <thead>
        <tr class="font13 white-space-nowrap">
            <th width="30">Item Head <small class="req" style="color:red;"> *</small></th>
            <!--<th width="30"><?php echo $this->lang->line('item_head'); ?><small class="req" style="color:red;"> *</small></th>-->
        ...
        <div class="modal-content modal-media-content mx-2">
            ...
            <form id="editformadd" action="<?php echo site_url('admin/itemcategory/edit') ?>" name="employeeform" method="post" accept-charset="utf-8" enctype="multipart/form-data">
                <div class="modal-body pt0 pb0">
                    <div class="ptt10">
                        <?php
$selected_item_head = set_value('item_head', isset($item_category['item_head']) ? $item_category['item_head'] : '');
?>
                        <?php echo $row['item_head'];  ?>
                        <div class="form-group">
                            <label for="item_head"><?php echo $this->lang->line('item_head') ?: 'Item Head'; ?></label><small class="req"> *</small>
                            <select id="item_head" name="item_head" class="form-control" required>
                                <option value=""><?php echo $this->lang->line('select') ?: 'Select'; ?></option>
                                <option value="Capital Equipment" <?php echo ($selected_item_head == 'Capital Equipment') ? 'selected' : ''; ?>>Capital Equipment</option>
                                <option value="Consumables" <?php echo ($selected_item_head == 'Consumables') ? 'selected' : ''; ?>>Consumables</option>
                            </select>
                            <span class="text-danger"><?php echo form_error('item_head'); ?></span>
                            <span class="text-danger"><?php echo form_error('item_head'); ?></span>
                        </div>
...
function addrow(){
    var id = total_rows+1;    
    var div = "<tr id='name_row_" + id + "'>" +
        // Item Head Dropdown
        "<td>" +
            "<select name='item_head_" + id + "' id='item_head_" + id + "' class='form-control'>" +
                "<option value=''>Select</option>" +
                "<option value='Capital Equipment'>Capital Equipment</option>" +
                "<option value='Consumables'>Consumables</option>" +
            "</select>" +
        "</td>" +
        // Item Category Text Input
        "<td>" +
            "<input class='form-control' name='cat_id_" + id + "' id='cat_id_" + id + "' value='' type='hidden' />" +
            "<input type='hidden' name='total_rows[]' value='" + id + "'>" +
            "<input name='itemcategory_" + id + "' id='itemcategory_" + id + "'  type='text' class='form-control' />" +
        "</td>" +
        // Description Input
        "<td><input name='description_" + id + "' id='description_" + id + "'  type='text' class='form-control'/></td>" +
        // Remove Button
        "<td><button type='button' data-rowid='" + id + "' class='closebtn delete_row'><i class='fa fa-remove'></i></button></td>" +
        "</tr>";
    $('#set_row').append(div);   
    total_rows++;      
}
